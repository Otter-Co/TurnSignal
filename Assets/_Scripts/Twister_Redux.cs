...
public class Twister_Redux : MonoBehaviour
{
    ...
    void UpdateArms()
    {
        // Have no idea if this is opted out,
        // But my intuition tells me It's better to declare here and reassign in the loop,
        // Then it is to re-declare every loop.
        Vector3[] armPoints = null;
        Vector3[] armLinePoints = null;
        GameObject armO = null;
        LineRenderer arm = null;
        Vector3 negaLocPos = Vector3.zero;
        ...
        for (int i = 0; i < armLines.Length; i++)
        {
            ...
            armPoints = points[i];
            armLinePoints = this.linePoints[i];
            ...
            negaLocPos = -(armO.transform.localPosition);

            armPoints[armPoints.Length - 1] = negaLocPos;

            UpdateArmPoints(armPoints, prog, -negaLocPos);
            UpdateLinePoints(armLinePoints, armPoints);

            arm.positionCount = armResolution;
            arm.SetPositions(armLinePoints);
        }
    }
